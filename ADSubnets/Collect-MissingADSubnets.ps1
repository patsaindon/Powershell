...
if ( ($LogEntries.Count -gt 0) -and ($CollectOnly -eq $false) )
{
	...
	$SubnetsArray | Export-Csv "$($Path)\ADSubnets-Export1.csv" -Delimiter ";" -NoTypeInformation -Force
	
	if ( Test-Path "$($Path)\ADSubnets-Export1.csv" )
	{
		Write-Host "$($Path)\ADSubnets-Export1.csv" -ForegroundColor Green
	}
	else
	{
...
Send-MailMessage -To $emailTo -From $emailFrom -Subject $subject -Attachments $attachment1,$attachment2 -SmtpServer $smtpServer

Remove-Item C:\Scripts\ADSubnets\ADSubnets-Export1.csv
Remove-Item C:\Scripts\ADSubnets\ADSubnets-MissingSubnets.csv
Remove-Item C:\Scripts\ADSubnets\*.log
