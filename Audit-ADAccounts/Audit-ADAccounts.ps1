...
try
{
	...
	elseif ( $PDCEmulators.Count -ge 1 )
	{
		...
		if ( !$CollectOnly )
		{
			...
			#make Excel visible 
			$script:Excel.Visible = $True
            While ([System.Runtime.Interopservices.Marshal]::ReleaseComObject($Excel)) {'cleanup Excel'}
		}
	}
	else
	{
...
Remove-Item "$Path\Report-UserAccounts.xlsx"
Remove-Item "$Path\SIQ-UserAccounts.csv"
